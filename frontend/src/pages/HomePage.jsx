...
export default function HomePage() {
  ...
  const demoOutfits = [
    {
      name: "Traditional Crimson Saree",
      color: "Scarlet Brocade Gold",
      garmentImg: "/media__1780075834904.jpg",
      modelImg: "/media__1780075834908.jpg",
      details: "Pure Silk Weave, Handcrafted Zari Border, Fluid Fall physics"
    },
    {
      name: "Pastel Mint Embroidered Kurta Set",
      color: "Mint Green Ivory",
      garmentImg: "/media__1780077825825.png",
      modelImg: "/media__1780342681756.png",
      details: "Embroidered Georgette weave, Mandarin collar, tailored fit"
    },
    {
      name: "Indo-Western Fusion Gown",
      color: "Royal Emerald Gold",
      garmentImg: "/media__1780343030538.jpg",
      modelImg: "/media__1783494205657.png",
      details: "Zari brocade top bodice, layered chiffon skirt, high-contrast wrap"
    }
  ];
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setIsFitting(true);
      setTimeout(() => {
        setActiveOutfitIdx((prev) => (prev + 1) % demoOutfits.length);
        setIsFitting(false);
      }, 1000); // 1s simulation transition
  ...
  return (
    <div className="min-h-screen bg-[#FFF8F8] pb-20 font-sans overflow-x-hidden relative">
      ...
      {/* ================= 5. AAVRITI LIVE SHOWROOM (Infinite Marquee) ================= */}
      <section className="relative z-20 w-full bg-white py-16 lg:py-20 overflow-hidden border-t border-pink-100/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-10 text-center lg:text-left">
          <ScrollReveal delay="100ms">
            <span className="text-[9px] font-black uppercase tracking-wider text-[#D81B60] bg-[#FFF0F2] px-3 py-1 rounded-full border border-[#F8D7DA]/60">Live Showroom</span>
            <h2 className="text-3xl font-black font-jakarta text-gray-900 tracking-tight mt-3">Continuously Updated Catalog Loop</h2>
            <p className="text-xs text-gray-500 font-medium leading-relaxed mt-1">
              Real-time uploads and designer coordinates passing through our automated cataloging queue.
            </p>
          </ScrollReveal>
        </div>

        {/* Infinite Loop Container */}
        <div className="relative w-full overflow-hidden flex py-4 bg-slate-50/50 border-y border-gray-100 select-none">
          {/* Main moving block */}
          <div className="flex gap-6 animate-marquee whitespace-nowrap shrink-0">
            {[...products, ...products].slice(0, 16).map((product, idx) => (
              <div 
                key={idx}
                className="w-48 bg-white border border-gray-150 p-3 rounded-2xl shadow-sm inline-flex flex-col gap-2 shrink-0 hover:border-[#D81B60]/30 hover:scale-[1.03] transition-all duration-300 cursor-pointer"
              >
                ...
                <div className="text-left">
                  <span className="block text-[8px] font-black uppercase tracking-wider text-[#D81B60]">{product.brand}</span>
                  <span className="block text-[11px] font-bold text-slate-800 truncate">{product.name}</span>
                  <span className="block text-xs font-black text-slate-900 mt-0.5">${product.price}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ================= 6. AAVRITI LIVE NEURAL MORPHER (Simulation Widget) ================= */}
      <section className="relative z-20 w-full bg-[#FFF8F8] py-16 lg:py-24 border-t border-pink-100/20">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            
            {/* Left Col: Explaining details (5 cols) */}
            <div className="col-span-12 lg:col-span-5 space-y-6 text-center lg:text-left">
              <ScrollReveal delay="100ms">
                <span className="text-[9px] font-black uppercase tracking-wider text-[#D81B60] bg-[#FFF0F2] px-3 py-1 rounded-full border border-[#F8D7DA]/60">Neural Draping Demo</span>
                <h2 className="text-3xl sm:text-4xl font-black font-jakarta text-gray-900 tracking-tight leading-none mt-3">
                  Live Fitting Demonstration
                </h2>
                <p className="text-xs text-gray-500 font-medium leading-relaxed mt-4">
                  Watch Aavriti AI warp, fold, and texture-map product flat lay images onto our curated body-shape avatars in real-time.
                </p>
              </ScrollReveal>

              <ScrollReveal delay="200ms" className="space-y-4 pt-2">
                {[
                  { label: "Fabric Physics Solver", val: "99.8% precision draping index" },
                  { label: "Rendering Speed", val: "Instant warp projection (~1.2s)" },
                  { label: "Shadow & Depth Shader", val: "Adaptive lighting coordinate matrix" }
                ].map((item, idx) => (
                  <div key={idx} className="flex items-center gap-3 justify-center lg:justify-start">
                    <div className="w-1.5 h-1.5 rounded-full bg-[#D81B60] animate-ping shrink-0" />
                    <span className="text-xs font-bold text-slate-800 uppercase tracking-wide">{item.label}:</span>
                    <span className="text-xs text-[#D81B60] font-extrabold">{item.val}</span>
                  </div>
                ))}
              </ScrollReveal>
            </div>

            {/* Right Col: Interactive Mock VTON simulator (7 cols) */}
            <div className="col-span-12 lg:col-span-7 flex justify-center">
              <ScrollReveal delay="300ms" className="w-full max-w-2xl bg-white border border-pink-100/50 p-6 rounded-[32px] shadow-xl relative overflow-hidden">
                
                {/* Ambient glow decoration */}
                <div className="absolute top-0 right-0 w-48 h-48 bg-[#D81B60]/5 rounded-full filter blur-2xl pointer-events-none" />

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-center">
                  
                  {/* Left: Input Garment Card */}
                  <div className="space-y-3 flex flex-col items-center">
                    <span className="text-[8px] font-black uppercase tracking-wider text-slate-400">01. Flat Lay Garment</span>
                    <div className="w-full aspect-[3/4] rounded-2xl overflow-hidden border border-gray-100 bg-gray-50/50 relative p-4 flex items-center justify-center">
                      <img 
                        src={demoOutfits[activeOutfitIdx].garmentImg} 
                        alt="Garment Flat" 
                        className={`max-w-full max-h-full object-contain drop-shadow-md transition-all duration-700 ${isFitting ? 'opacity-40 blur-xs scale-98' : 'opacity-100 scale-100'}`}
                      />
                      {/* Laser scanner element */}
                      <div className="absolute left-0 w-full h-[2px] bg-[#D81B60] shadow-[0_0_10px_#D81B60] opacity-75 animate-laser" />
                    </div>
                    <div className="text-center">
                      <span className="block text-xs font-bold text-slate-800 truncate">{demoOutfits[activeOutfitIdx].name}</span>
                      <span className="block text-[10px] text-gray-400 mt-0.5">{demoOutfits[activeOutfitIdx].color}</span>
                    </div>
                  </div>

                  {/* Right: Output Fitted Model Card */}
                  <div className="space-y-3 flex flex-col items-center">
                    <span className="text-[8px] font-black uppercase tracking-wider text-[#D81B60] flex items-center gap-1.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-[#D81B60] animate-ping" />
                      02. AI Fitted Simulation
                    </span>
                    <div className="w-full aspect-[3/4] rounded-2xl overflow-hidden border border-pink-100/40 bg-pink-50/5 relative p-1 flex items-center justify-center shadow-inner">
                      <img 
                        src={demoOutfits[activeOutfitIdx].modelImg} 
                        alt="Model Drape" 
                        className={`w-full h-full object-cover rounded-xl transition-all duration-700 ${isFitting ? 'opacity-30 blur-xs scale-[1.01]' : 'opacity-100 scale-100'}`}
                      />
                      {isFitting && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center bg-white/20 backdrop-blur-xs rounded-xl">
                          <Loader2 className="w-7 h-7 text-[#D81B60] animate-spin" />
                          <span className="text-[9px] font-black text-[#D81B60] uppercase tracking-widest mt-2 animate-pulse">Calculating mesh...</span>
                        </div>
                      )}
                    </div>
                    <div className="text-center px-4">
                      <span className="block text-xs font-extrabold text-[#D81B60] uppercase tracking-wide">Ready for Checkout</span>
                      <span className="block text-[10px] text-gray-500 leading-tight mt-0.5 line-clamp-1">{demoOutfits[activeOutfitIdx].details}</span>
                    </div>
                  </div>
...
// Icon helper components for fallback
